...
const PaginationView = ({...props}: PaginationViewProps) => {
  ...
  return <PaginationContainerStyle>
    ...
    {
      page !== 1 && <PaginationButtonStyle onClick={()=>handelArrowPage('prev')}><BiChevronLeft/></PaginationButtonStyle>

    }
    {
      pageArray.map((item, index) => {
        return <PaginationButtonStyle className={page === item ? 'active' : '' } onClick={()=>handelPageChange(item as number)} key={index}>{item}</PaginationButtonStyle>
      })
    }
    {
      page !== pageTotal && <PaginationButtonStyle onClick={()=>handelArrowPage('next')}><BiChevronRight/></PaginationButtonStyle>
    }
